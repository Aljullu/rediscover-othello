...
var Board = function (mapSettings) {
  this.cellRows = mapSettings.cellRows
    ? mapSettings.cellRows
    : preferences.getSetting("cellRows");
  this.cellColumns = mapSettings.cellColumns
    ? mapSettings.cellColumns
    : preferences.getSetting("cellColumns");
  ...
  this.boardColor = mapSettings.boardColor
    ? mapSettings.boardColor
    : preferences.getSetting("boardColor");
  this.borderColor = mapSettings.borderColor
    ? mapSettings.borderColor
    : preferences.getSetting("boardBorderColor");

  this.startingPointX = mapSettings.startingPointX
    ? mapSettings.startingPointX
    : preferences.getSetting("startingPointX");
  this.startingPointY = mapSettings.startingPointY
    ? mapSettings.startingPointY
    : preferences.getSetting("startingPointY");
...
Board.prototype.initialize = function () {
  ...
  if (this.mapW && this.mapB) {
    ...
    if (
      typeof this.startingPointX !== "undefined" &&
    ...
    if (
      typeof this.startingPointY !== "undefined" &&
...
Board.prototype.draw = function () {
  if (this.toBePainted) {
    ...
    ctx.fillRect(
      0,
      0,
      this.cellColumns * this.cellWidth,
      this.cellRows * this.cellHeight
    );
...
Board.prototype.updateInfo = function () {
  ...
  if (!this.canPlayerPlay(this.playerPlaying)) {
    ui.showMessage(
      "Player " + this.playerPlaying + " cannot move, he looses his turn."
    );
    ...
    if (!this.canPlayerPlay(this.playerPlaying)) {
      ui.showMessage(
        "Player " +
          this.playerPlaying +
          " cannot move either. The game is over."
      );
      noMoreMoves = true;
    }
  }
