...
UI.prototype.writePlayerName = function (id) {
  $("#player" + id + " .name").html(
    '<i class="fa"></i> ' + this.playerNames[id]
  );
};
...
UI.prototype.startCampaign = function () {
  ...
  $("#campaign-level")
    .text(parseInt(this.campaignLevel, 10) + 1)
    .removeClass("hidden");
  ui.prepareMap();
};
...
UI.prototype.showBestScores = function () {
  if (localStorage.getItem("bestScores")) {
    ...
    $(".best-scores-table").html(
      "<tr><th>Date</th><th>Points</th><th>Duration</th></tr>"
    );
    for (var i = 0; i < bestScores.length; i++) {
      $(".best-scores-table").append(
        "<tr><td>" +
          bestScores[i].date +
          "</td><td>" +
          bestScores[i].player1 +
          "-" +
          bestScores[i].player2 +
          " (" +
          bestScores[i].advantage +
          ")</td><td>" +
          bestScores[i].duration +
          "s.</td></tr>"
      );
    }
...
UI.prototype.savePreferences = function () {
  ...
  var boardBorderColorInput = document.getElementById(
    "preferences-board-border-color"
  );
  ...
  var startingPointXInput = document.getElementById(
    "preferences-startingPointX"
  );
  var startingPointX = startingPointXInput.value;
  preferences.saveSetting("startingPointX", startingPointX);

  var startingPointYInput = document.getElementById(
    "preferences-startingPointY"
  );
...
UI.prototype.updateStartingPointVisualization = function () {
  if ($("#different-starting-point").is(":checked")) {
    ...
    if ($("#preferences-startingPointX").val() === "") {
      $("#preferences-startingPointX").val(
        Math.floor($("#preferences-cellRows").val() / 2)
      );
    }
    if ($("#preferences-startingPointY").val() === "") {
      $("#preferences-startingPointY").val(
        Math.floor($("#preferences-cellColumns").val() / 2)
      );
    }
  } else {
